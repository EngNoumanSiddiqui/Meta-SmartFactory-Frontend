...
@Component({
  selector: 'app-daily-rework-graph',
  templateUrl: './dail-rework-graph.component.html',
  styleUrls: ['./daily-rework-graph.component.scss']
})
export class DailyReworkGraphComponent implements OnInit {
  ...
  @Input('filterModel') set f(filterModel) {
    if (filterModel && filterModel.startDate && filterModel.finishDate) {
      this.loadReworkCause(filterModel);
    }

  }

  constructor(private dashboardService: DashboardService, 
    private utilities: UtilitiesService, private datePipe: DatePipe) {

  }

  loadReworkCause(filterModel) {

    this.dashboardService.getDailyReworkPercentageList(filterModel).then(res => {
      this.initChart(res);
    }).catch(err => {
