...
import { CreateNewProdObject, JobOrderList, JobOrderStockUseList } from 'app/dto/porder/porder.model';
...
import { ProductTreeService } from '../../../../../services/dto-services/product-tree/prod-tree.service';

@Component({
  selector: 'app-sale-review-job-order',
  templateUrl: './sale-review-job-order.component.html',
  styleUrls: ['./sale-review-job-order.component.scss']
})
export class SaleReviewJobOrderComponent implements OnInit, OnDestroy {
  ...
  addmodal = { active: false, rowData: null, title: null, mode: null, stockIndex: null, index: null, direction: null, data: null };
  ...
  @Input('salesSelectedOrder') salesSelectedOrder = null;
        ...
        this.jobOrderls.forEach((jb, jbIndex) => {
          jb.jobOrderOperations.forEach((op, index) => {
            if (!op.operation && op.operationId) {
              op.operation = <any>{ operationName: op.operationName, operationId: op.operationId, location: op.location }
            }
            ...
            // if(this.prodData.prodOrderStatus=== 'WAITING_INITIAL_REVIEW') {
            //   op.workStation = null;
            //   op.workStationId = null;
            //   op.workStationName = null;
            //   op.fixedWorkstation = false;
            // }
          });
  ...
  setSelectedMaterial(event, index) {
    if (event) {
      ...
      this.jobOrderList[0].jobOrderOperations[0].jobOrderStockUseList.forEach((itm, i) => {
        if (i === index) {
  ...
  // okWeight(weightPanel?: any) {
  //   this.selectedProdItemMaterial.weight=(this.selectedProdItemMaterial.length * this.selectedProdItemMaterial.innerDiameter * this.selectedProdItemMaterial.outerDiameter * 3.14 * this.selectedProdItemMaterial.density) / 4000000;
  //   this.selectedProdItemOperation.variableCost = this.selectedProdItemMaterial.weight * this.selectedProdItemOperation.variableCostRate;
  //   this.selectedProdItemOperation.laborCost = this.selectedProdItemMaterial.weight * this.selectedProdItemOperation.laborCostRate;
  //   if(weightPanel) weightPanel.hide();
  // }

  // changeWeight(item, operation?: any) {
  //   // item.weight = (item.length * item.innerDiameter * item.outerDiameter * 3.14 * item.density) / 400000;
  //   operation.variableCost = item.weight * operation.variableCostRate;
  //   operation.laborCost = item.weight * operation.laborCostRate;
  //   // if(weightPanel) weightPanel.hide();
  // }
  ...
  // onMaterialCostChange(item) {
  //   item.materialCost=(item.quantity*this.orderQuantity*item.materialCostRate)
  // }
  addMaterial() {
    let material = {
      actualMaterialCost: null,
      ...
      jobOrderOperations: [
        {
          ...
          jobOrderId: null,
          jobOrderOperationId: null,
          referenceId: this.salesSelectedOrder?.referenceId,
          jobOrderStockAuxList: [],
  ...
  addJobOrderParallelItem(rowNode, parallel = false, parent = false) {
    ...
    if (parallel) {
      jobOrder.orderNo = parseInt(this.selectedRowNode.parent.children[this.selectedRowNode.parent.children.length - 1].data.orderNo) + 1;
      ...
      if (this.selectedRowNode?.parent?.children && this.selectedRowNode.parent.children.length > 0) {
        let quantity = 0;
        this.selectedRowNode.parent.children.forEach(itm => {
          itm.data?.jobOrderOperations.forEach(itm2 => {
            itm2.jobOrderStockProduceList.forEach(itm3 => {
              quantity += itm3.quantity;
            });
          });
        });
        quantity += jobOrder.jobOrderOperations[0].jobOrderStockProduceList[0].quantity;
      }
  ...
  backToSalesOrder() {
    this._confirmationSvc.confirm({
      message: this._translateSvc.instant('are-you-sure-go-back-without-saving'),
      header: this._translateSvc.instant('back-confirmation'),
      icon: 'fa fa-trash',
      accept: () => {
        this.backAction.next('back');
      },
      reject: () => {
  ...
  async savingAllOperations(jobOrderOperations) {
    ...
    this.prodData.prodOrderStatus = 'WAITING_FINAL_REVIEW';
    this._loaderSvc.showLoader();
    this.counter = this.counter + 1;
    if (this.salesSelectedOrder) {
    ...
    try {
      ...
      if (this.prodData.orderDetail && !this.prodData.orderDetailId) {
        this.prodData.orderDetailId = this.prodData.orderDetail?.orderId;
      }
      await this._prodOrderSvc.save(this.prodData);
  ...
  addOrUpdatecomponent(item) {
    if (this.addmodal.data) {// edit event
      ...
      this.addmodal.data.description = item.description;
      ...
      this.addmodal.data.stock = this.addmodal.data.component;
      this.addmodal.data.stockId = this.addmodal.data.component?.stockId;
      this.addmodal.data.stockName = this.addmodal.data.component?.stockName;
      this.addmodal.data.stockNo = this.addmodal.data.component?.stockNo;
      delete this.addmodal.data.component;

      // this.addmodal.rowData.productTreeDetailComponentId = item.productTreeDetailComponentId;
      if (this.addmodal.stockIndex != null) {
        if (this.addmodal.mode === 'component') {
          this.addmodal.rowData.jobOrderOperations[this.addmodal.index].jobOrderStockUseList[this.addmodal.stockIndex] = { ...this.addmodal.data };
        } else {
          this.addmodal.rowData.jobOrderOperations[this.addmodal.index].jobOrderStockProduceList[this.addmodal.stockIndex] = { ...this.addmodal.data };
        }
      } else {
        if (this.addmodal.mode === 'component') {
          this.addmodal.rowData.jobOrderOperations[this.addmodal.index].jobOrderStockUseList.push({
            ...
            currentQuantity: null,
            currentStockQuantity: null,
            description: this.addmodal.data.description,
            currentStockReservedQuantity: null,
            cycleRate: null,
            defectName: null,
            defectQuantity: null,
            ...
            quantityUnit: this.addmodal.data.component?.baseUnit,
            useStock: null,
            wareHouseStockId: null,
