...
@Component({
  selector: 'app-summarized-stock-reports',
  templateUrl: './summarized-stock-reports.component.html',

})
export class SummarizedStockReportComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.searchTerms.pipe(
      debounceTime(400),
      switchMap(term => this.stockCardService.localStockFilterFrontEndStockSummary(term))).subscribe(
        result => {
  ...
  exportCSV(selected: boolean = false, type: BookType = 'csv') {
    if(selected) {
      ...
      this.loaderService.showLoader();
      this.stockCardService.localStockFilterFrontEndStockSummary({...this.pageFilter, pageNumber: 1, pageSize: this.pagination.totalElements})
      .subscribe(result => {
