import { Component, Input, OnInit, EventEmitter, Output, ViewEncapsulation, ViewChildren, QueryList } from '@angular/core';
...
export class DetailStockTransferNotificationComponent implements OnInit {
  ...
  @Input('data') set d(data){
    if(data){
      ...
      this.stockTransferReceipt.responseStockTransferNotificationDetailList.forEach(itm => {
        ...
        if(this.stockTransferReceipt.goodsMovementActivityType === 'GOODS_RECEIPT' 
          && this.stockTransferReceipt.goodMovementDocumentType === 'PURCHASE_ORDER') {
            if(itm.wareHouseFromId) {
              itm.wareHouseToId = itm.wareHouseFromId;
              itm.wareHouseToName = itm.wareHouseFromName;
              // itm.wareHouseFromId = null;
              // itm.wareHouseFromName = null;
            }
  ...
  setStockTransferReceipt(){
    ...
    //     this.stockTransferReceipt.responseStockTransferNotificationDetailList.forEach((item, index) => {
    //       this.imageAdderComponent.toArray()[index].initImages(item.stockTransferNotificationDetailId,
    //          TableTypeEnum.GOODS_MOVEMENT_DETAIL);
    //     });
  ...
  showBarCode(barcode) {
    this.loaderService.showDetailDialog(DialogTypeEnum.BARCODE, barcode);
  }
