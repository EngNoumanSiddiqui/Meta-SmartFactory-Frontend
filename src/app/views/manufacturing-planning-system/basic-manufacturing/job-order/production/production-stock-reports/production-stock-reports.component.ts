...
@Component({
  selector: 'app-production-stock-reports',
  templateUrl: './production-stock-reports.component.html'
})
export class ProductionStockReportsChartComponent implements OnInit {


  @Input() plantId = null;
  @Input() prodOrderId = null;
  ...
  ngOnInit() {
    ...
    this.lineChartOptions = this.createOptions(name);
    this.shiftBasedStockReportPageFilter.finishDate = this.finishDate;
    this.shiftBasedStockReportPageFilter.startDate = this.startDate || new Date();
    this.shiftBasedStockReportPageFilter.plantId = this.plantId;
    this.shiftBasedStockReportPageFilter.prodOrderId = this.prodOrderId;
    this.shiftBasedStockReportPageFilter.jobOrderId = this.jobOrderId;

    if(this.shiftBasedStockReportPageFilter.finishDate && this.shiftBasedStockReportPageFilter.plantId 
      && (this.shiftBasedStockReportPageFilter.prodOrderId || this.shiftBasedStockReportPageFilter.jobOrderId)) {
      this.filtershiftBasedStockReport();
    }
  ...
  filtershiftBasedStockReport() {
    ...
    Promise.all([
    this.stockCardService.getTrendAllProdStockWarehouseShiftProdJobReport(temp).toPromise(),
    ]).then(res => {
  ...
  initializeWeeklyChart() {
    if (!this.trendStockWarehouseShiftReportData.weeklyList || this.trendStockWarehouseShiftReportData.weeklyList.length === 0) {
  ...
  initializeDailyChart() {
    if (!this.trendStockWarehouseShiftReportData.dailyList || this.trendStockWarehouseShiftReportData.dailyList.length === 0) {
      this.dailyChartData = {
        labels: [],
        datasets: []
      }
  ...
  initializeMonthlyChart() {
    if (!this.trendStockWarehouseShiftReportData.montlyList || this.trendStockWarehouseShiftReportData.montlyList.length === 0) {
