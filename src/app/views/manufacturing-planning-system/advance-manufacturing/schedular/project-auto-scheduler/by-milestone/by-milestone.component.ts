...
@Component({
  selector: 'app-by-milestone',
  templateUrl: './by-milestone.component.html',
  styleUrls: ['./by-milestone.component.scss']
})
export class ByMilestoneComponent implements OnInit, OnChanges {
    ...
    if(simpleChanges.Data && simpleChanges.Data.currentValue) {
      ...
      if (this.responseLaborSchedular && this.responseLaborSchedular.length > 0) {
        ...
        this.responseLaborSchedular.forEach(item => {
          // const project = this.resources.find(itm => itm.id === item.projectId);
          // if (!project) {
          //   const resourse = {
          ...
          item.milestoneList.forEach(ml => {
            ...
            const aEvent = {
              ...
              name: '#pc= '+ item.code + ', #pn= ' + item.name + ', #mc= '+ml.code + ', #mn= ' + ml.name,
              status: ml.status,
