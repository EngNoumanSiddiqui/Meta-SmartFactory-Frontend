...
@Component({
  selector: 'app-by-prod-orders',
  templateUrl: './by-prod-orders.component.html',
  styleUrls: ['./by-prod-orders.component.scss']
})
export class ByProdOrdersComponent implements OnInit, OnChanges {
    ...
    if(simpleChanges.Data && simpleChanges.Data.currentValue) {
      ...
      if (this.responseLaborSchedular && this.responseLaborSchedular.length > 0) {
        ...
        this.responseLaborSchedular.forEach(item => {
          ...
          if(item.milestoneList) {
            item.milestoneList.forEach(ml => {
              if(ml.prodOrderList) {
                ml.prodOrderList.forEach(prod => {
                  if (minDate === 0) {
                  ...
                  const aEvent = {
                    ...
                    name: '#rfId= '+ prod.referenceId + ', #prodId= ' + prod.prodOrderId + ', #Qt= ' + prod.quantity,
