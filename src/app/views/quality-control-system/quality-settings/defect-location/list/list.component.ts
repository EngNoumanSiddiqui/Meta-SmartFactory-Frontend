...
export class ListDefectLocations implements OnInit, OnDestroy {

  defectLocationTypeModal = {
    modal: null,
    data: null,
    id: null
  };
  ...
  selectedColumns = [
    { field: 'defectLocationId', header: 'defect-location-id'},
    { field: 'defectLocationCode', header: 'code'},
    { field: 'text', header: 'defect-location'},
  ];
  cols = [
    { field: 'defectLocationId', header: 'defect-location-id'},
    { field: 'defectLocationCode', header: 'code'},
  ...
  pageFilter = {
    ...
    defectLocation: null,
    query: null,
    orderByProperty: 'defectLocationId',
  ...
  modalShow(id, mod: string) {
    this.defectLocationTypeModal.id = id;
    this.defectLocationTypeModal.modal = mod;
    this.modal.active = true;
  }

  modalClone(mod: string, data) {
    this.defectLocationTypeModal.modal = mod;
    this.defectLocationTypeModal.data = data[0].stockId;
    this.modal.active = true;
  }
  onSaveSuccessful(event, myModal) {
    ...
    if (this.isSaveAndNew) {
      this.modal.active = true;
      this.defectLocationTypeModal.modal = 'NEW';
      myModal.show();
    }
    this.defectLocationTypeModal.id = null;
    this.isSaveAndNew = false;
    this.selectedDefectLocationTypes = null;
  }
  constructor(
  ...
  ngOnInit() {
    this.defectLocationTypes = [];
    // this.loaderService.showLoader();

    this.searchTerms.pipe(
      debounceTime(400),
      switchMap(term => this._defectLocationsService.filter(term))).subscribe(
      result => {
        this.pagination.currentPage = result['currentPage'];
        this.pagination.totalElements = result['totalElements'];
        this.pagination.totalPages = result['totalPages'];
        this.defectLocationTypes = result['content'];
  ...
  delete(id) {
    this._confirmationSvc.confirm({
      ...
      key: 'quality-causes',
      accept: () => {
          this._defectLocationsService.delete(id).then(
            result => {
              this.pagination.currentPage = result['currentPage'];
              this.pagination.totalElements = result['totalElements'];
              this.pagination.totalPages = result['totalPages'];
